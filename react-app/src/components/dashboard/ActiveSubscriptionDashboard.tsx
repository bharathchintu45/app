               ...
               <div className="space-y-2">
                 ...
                 {(() => {
                    const activeMeals = plan.allowedSlots
                      .filter((s: Slot) => !selectedDayHold.day && !selectedDayHold.slots[s])
                      .map((s: Slot) => selectedDayPlan[s])
                ...
                return (
                  <div key={s} className={cn(
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 p-3 md:p-6">
                      <div className={cn(
                        "w-12 h-12 sm:w-14 sm:h-14 md:w-24 md:h-24 rounded-lg md:rounded-2xl shrink-0 overflow-hidden shadow-md",
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1 mb-1">
                           <div className="p-0.5 sm:p-1 rounded-md bg-slate-100 text-slate-500 shrink-0"><SlotIcon size={10} className="sm:w-3 sm:h-3" /></div>
                           <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.1em] text-slate-400 truncate">{slotLabel(s)}</span>
                        </div>
                        ...
                        {!selectedDayHold.day && (
                          <button
                            ...
                            className={cn(
                              ...
                              selectedDayHold.slots[s] ? "bg-rose-100 text-rose-600 hover:bg-rose-200" : "bg-slate-100 text-slate-500 hover:bg-slate-200"
                            )}
                          >
                            {selectedDayHold.slots[s] ? "Unhold Meal" : "Hold Meal"}
                          </button>
                        )}
                        {!isMealHeld && onSwapMeal && (
                          <button
                            onClick={() => onSwapMeal(s, selectedDate)}
