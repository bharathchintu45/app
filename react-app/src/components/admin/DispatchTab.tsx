...
export default function DispatchTab({ showToast }: { showToast: (msg: string) => void }) {
  ...
  async function assignOrder(orderId: string, boyId: string) {
    ...
    const { error } = await api.v1.manageDispatch({
      action: 'assign_order',
      orderId,
      boyId
    });

    if (error) showToast("Error assigning order: " + error.message);
    else { 
  ...
  function TeamManager() {
    ...
    async function handleAddBoy(e: any) {
      ...
      const { error } = await api.v1.manageDispatch({
        action: 'add_boy',
        data: { name: nbName, phone: nbPhone, vehicle: vehicleDesc, profileId: nbProfileId }
      });
